...
class Geocoding
{
     ...
     * @throws \Exception
     */
    public static function geocode(string $searchQuery): Collection
    {
        $httpClient = new Client();
        $nominatimUrl = 'https://nominatim.openstreetmap.org/search.php?q=' . urlencode($searchQuery) . '&polygon_geojson=1&format=jsonv2';
        try {
            // Make the GET request to Nominatim API
            $response = $httpClient->get($nominatimUrl);
            $geoResults = json_decode($response->getBody(), true);
            // Extract latitude and longitude from the first element of the array
            if (!empty($geoResults) && isset($geoResults[0])) {
                $latitude = $geoResults[0]['lat'];
                $longitude = $geoResults[0]['lon'];
                // Return a collection with latitude and longitude
                return collect([
                    'latitude' => $latitude,
                    'longitude' => $longitude,
                ]);
            } else {
                return collect(); // Return an empty collection if no results are found
            }
        } catch (\Exception $e) {
